...
1) **Profile input (Quick mode):** user enters name, location, language, capital, time per week (5–7 fields in Quick; Advanced adds assets). States: draft → ready. (Money_Map_Spec_Packet.pdf p.4, p.8)
2) **Recommendations (Top-N):** system ranks variants with feasibility, economics (time-to-first-money + net range + costs), legal gate, and explanations; objective is selected as a preset here. States: loading → empty → results. Errors: “no results” when filters exclude all. (Money_Map_Spec_Packet.pdf p.4, p.6–8)
...
2) **Profile input (Quick + Advanced) flow**  
   **Acceptance criteria:**  
   ...
   - Profile state transitions draft → ready and uses required MVP inputs (name/location/time/capital/language; assets in Advanced; objective preset is on Recommendations). (Money_Map_Spec_Packet.pdf p.4, p.8)  
...
- Release 0.1 emits placeholder artifacts under `exports/artifacts/` (checklist.md, budget.yaml, outreach_message.txt) and always includes a “Prep tasks” step; see `docs/DECISIONS.md` (2026-02-03). (Money_Map_Spec_Packet.pdf p.4, p.6–7)
