...
1) **Data status + validate + staleness flow**  
   **Acceptance criteria:**  
   - Data status section exists and displays dataset_version, reviewed_at, warnings, and a validate report. (Money_Map_Spec_Packet.pdf p.8, p.15)  
...
6) **Export + auditability flow**  
   **Acceptance criteria:**  
   - Exports include plan.md, result.json, and profile.yaml. (Money_Map_Spec_Packet.pdf p.10)  
   - result.json includes applied_rules (rule ids + reasons) or equivalent audit trace. (Money_Map_Spec_Packet.pdf p.11)  

7) **(Optional) CLI smoke flow**  
   **Acceptance criteria:**  
   - CLI supports validate → recommend → plan → export in a short demo run. (Money_Map_Spec_Packet.pdf p.14)  
...
- Data status section (dataset_version, reviewed_at, warnings, validate report) with valid/invalid/stale states. (Money_Map_Spec_Packet.pdf p.8, p.15)
