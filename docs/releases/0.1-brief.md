...
1) **Profile input (Quick mode):** user enters location, time, capital, language, assets, constraints (5–7 fields in Quick; Advanced adds more). States: draft → ready. (Money_Map_Spec_Packet.pdf p.4, p.8)
...
2) **Profile input (Quick + Advanced) flow**  
   **Acceptance criteria:**  
   ...
   - Profile state transitions draft → ready and uses required MVP inputs (location/time/capital/language/assets/constraints; objective preset is on Recommendations). (Money_Map_Spec_Packet.pdf p.4, p.8)  
